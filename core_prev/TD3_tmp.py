...
class TD3(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100, update_actor=False, weight_factor=0) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            replay_data = self.expert_replay_buffer.sample(batch_size, env=self._vec_normalize_env)
            ...
            with th.no_grad():
                ...
                # target Q is used to predict the N_th <s_n, a_n> after current <s0,a0>, n is the size of boostrapping window
                ...
                # calculate the n-step boostrapping + Q(s_n, a_n)
                target_q_real_nstep = constrained_replay_data.nstep_reward + (1 - constrained_replay_data.nth_dones) * constrained_replay_data.nstep_gamma * nth_targets_q_constrained_
                # calculate the m-step boostrapping + nstep Q_optimal + Q_target
                target_q_sub_nstep = constrained_replay_data.subopt_values + (1 - constrained_replay_data.nth_dones) * constrained_replay_data.nstep_gamma * nth_targets_q_constrained_
                # calculate the lower bound
                #target_q_lower_bound, _ = th.max(th.cat((target_q_real_nstep, target_q_sub_nstep), dim=1),dim=1, keepdim=True)
                target_q_lower_bound = target_q_real_nstep
                ...
                target_q_upper_bound = 5 + (1 - constrained_replay_data.dones) * self.gamma * targets_q_constrained
                #target_q_lower_bound, _ = th.max(th.cat((target_q_lower_bound, targets_q_constrained), dim=1),dim=1, keepdim=True)
                constrained_mask = (targets_q_constrained > target_q_lower_bound) * (targets_q_constrained < target_q_upper_bound)
            ...
            # Monte-Carlo prediction for nstep Q value (Supervised): Q = sum(r_i) + Q(s_m, a_m)
            ...
            # The optimal tightening version for critic loss
            ...
            #""" first version of lower-bound loss, clip the Q
            zero_tensors = th.zeros(constrained_current_q_estimates1.shape)
            lower_bound_filtered_q_dis1, _ = th.max(th.cat(((target_q_lower_bound - constrained_current_q_estimates1), zero_tensors), dim=1),dim=1, keepdim=True)
            lower_bound_filtered_q_dis2, _ = th.max(th.cat(((target_q_lower_bound - constrained_current_q_estimates2), zero_tensors), dim=1),dim=1, keepdim=True)
            ...
            # """ second version of lower-bound loss, first clip Q_target, then apply to MSE(Q, r+Q_target)
            ...
            # the standard lower bound q value for loss
            zero_tensors = th.zeros(constrained_current_q_estimates1.shape)
            standard_lower_bound_q_dis1, _ = th.max(th.cat(((target_q_real_nstep - constrained_current_q_estimates1), zero_tensors), dim=1),dim=1, keepdim=True)
            standard_lower_bound_q_dis2, _ = th.max(th.cat(((target_q_real_nstep - constrained_current_q_estimates2), zero_tensors), dim=1),dim=1, keepdim=True)
            critic_loss_low_standard1 = th.mean(standard_lower_bound_q_dis1**2)
            critic_loss_low_standard2 = th.mean(standard_lower_bound_q_dis2**2)
            critic_loss_standard_lower = sum([critic_loss_low_standard1, critic_loss_low_standard2])

            #print(self.bound_type)
            if self.bound_type is None or self.bound_type=='None':
                critic_loss = critic_loss_origin
            elif self.bound_type == 'DDPGfD':
                critic_loss = critic_loss_origin + 0.5*critic_loss_expert #+ 0.*critic_loss_standard_lower + 0.4*critic_loss_constrained
            elif self.bound_type == 'standard_lower_bound': # generalized SIL
                critic_loss = critic_loss_origin + 0.5*critic_loss_standard_lower 
            elif self.bound_type == 'expert_lower_bound':
                critic_loss = critic_loss_origin + 0.5*critic_loss_low_constrained
            elif self.bound_type == 'expert_upper_bound':
                critic_loss = critic_loss_origin + 0.5*critic_loss_upper_constrained
            elif self.bound_type == 'target_bound':
                critic_loss = critic_loss_origin + 0.5*critic_loss_constrained_target
            elif self.bound_type == 'hybrid':
                critic_loss = critic_loss_origin + 0.5*critic_loss_expert + 0.4*critic_loss_constrained
