    ...
    def train(self, gradient_steps: int, batch_size: int = 100, update_actor=False, weight_factor=0, use_expert_Q=True) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                targets_q = constrained_replay_data.rewards + (1 - constrained_replay_data.dones) * self.gamma * target_q
                ...
                target_q_sub_nstep = constrained_replay_data.subopt_values + \
                    (1 - constrained_replay_data.nth_dones) * constrained_replay_data.nstep_gamma * nth_targets_q_constrained_

                targets_q_constrained = target_q.clone().detach()

                #target_q_upper_bound = constrained_replay_data.optimal_values + \
                #    (1 - constrained_replay_data.dones) * constrained_replay_data.nstep_gamma * nth_targets_q_constrained_
                target_q_upper_bound = constrained_replay_data.optimal_values + \
                    (1 - constrained_replay_data.dones) * constrained_replay_data.nstep_gamma * nth_targets_q_constrained_
                ...
                target_q_lower_bound = target_q_lower_bound * (1 - constrained_replay_data.nth_dones)
            ...
            constrained_current_q_estimates = self.critic(
                constrained_replay_data.observations.float(), constrained_replay_data.actions.float())
            ...
            """
            critic_loss_constrained_target = sum([F.mse_loss(current_q, targets_q_constrained_masked)
                                                  for current_q in constrained_current_q_estimates])
