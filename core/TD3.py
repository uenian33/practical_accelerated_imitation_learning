    ...
    def train(self, gradient_steps: int, batch_size: int = 100, update_actor=False, weight_factor=0, use_expert_Q=True) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                #constrained_replay_data.nstep_reward + (1 - constrained_replay_data.nth_dones) * constrained_replay_data.nstep_gamma * nth_targets_q
                ...
                #constrained_replay_data.subopt_values +  (1 - constrained_replay_data.nth_dones) * constrained_replay_data.nstep_gamma * nth_targets_q
                ...
                if self.bound_type=='constrained_lower':
                    ...
                    targets_q_constrained_lower = (1 - constrained_mask_lower) * target_q + constrained_mask_lower * target_q_lower_bound
                    ...
                    targets_q_constrained_lower = (1 - constrained_mask_lower) * target_q + constrained_mask_lower * target_q_real_nstep
                    targets_q_constrained_masked = targets_q_constrained_lower
                    ...
                    constrained_mask_upper = th.logical_not(target_q > target_q_upper_bound).type(th.float) 
                    targets_q_constrained_lower = (1 - constrained_mask_lower) * target_q + constrained_mask_lower * target_q_lower_bound
                    targets_q_constrained_upper = constrained_mask_upper * targets_q_constrained_lower + (1 - constrained_mask_upper) * target_q_upper_bound
                    ...
                    constrained_mask_lower = th.logical_not(target_q < target_q_real_nstep).type(th.float)  
                    constrained_mask_upper = th.logical_not(target_q > prev_nth_targets_q).type(th.float) 
                    targets_q_constrained_lower = (1 - constrained_mask_lower) * constrained_mask_lower * target_q +  constrained_mask_lower * target_q_real_nstep
                    targets_q_constrained_upper = constrained_mask_upper * target_q + (1 - constrained_mask_upper) * prev_nth_targets_q
            ...
            if self.bound_type=='none' or self.bound_type is None or self.bound_type=='None':
                ...
                critic_loss = 0.7*critic_loss_origin + 0.2*critic_loss_expert + 0.1*critic_loss_nstep
                ...
                critic_loss = 0.7*critic_loss_origin + 0.2*critic_loss_expert + 0.3*critic_loss_constrained_target
                ...
                critic_loss_constrained_target = sum([F.mse_loss(current_q, targets_q_constrained_masked) for current_q in current_q_estimates])
                #print(critic_loss_lower, critic_loss_upper, critic_loss_constrained_target)
                critic_loss = 0.7*critic_loss_origin + 0.2*critic_loss_expert + 0.3*critic_loss_constrained_target #+ 0.15*critic_loss_constrained_target
                """
                critic_loss_constrained_target = sum([F.mse_loss(current_q, targets_q_constrained_masked) for current_q in current_q_estimates])
