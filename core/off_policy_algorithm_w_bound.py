...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        self.wasserstein_replay_buffer = IdealReplayBuffer(
            self.buffer_size,
            self.observation_space,
            self.action_space,
            self.device,
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            ...
            rollout, trajectories = self.collect_bounded_wasserstein_rollouts(
                self.env,
    ...
    def collect_rollouts(
        ...
        return self.collect_bounded_wasserstein_rollouts(env, callback, n_episodes, n_steps, action_noise, learning_starts,
    ...
    def collect_acceleration_rollouts(
        ...
        while total_steps < n_steps or total_episodes < n_episodes:
            ...
            while not done:
                ...
                if self.constrained_replay_buffer is not None:
    ...
    def collect_bounded_wasserstein_rollouts(
        self,
        env: VecEnv,
        callback: BaseCallback,
        n_episodes: int = 1,
        ...
        while total_steps < n_steps or total_episodes < n_episodes:
            ...
            while not done:
                ...
                if wasserstein_reward < imitation_reward:
                    wasserstein_reward = imitation_reward
                    print(wasserstein_reward, imitation_reward)
                reward = imitation_reward
                #env.render()
                if self.intermediate_max_r < imitation_reward:
                    self.intermediate_max_r = imitation_reward
                ...
                if replay_buffer is not None:
                    ...
                    #print(self.critic_target(th.tensor(self._last_original_obs).float(), th.tensor(buffer_action).float()))
                    replay_buffer.add(self._last_original_obs, new_obs_, buffer_action, imitation_reward, done,
                                      ideal_obs=self._last_original_obs, 
                                      ideal_next_obs=new_obs_,
                                      ideal_action=buffer_action,
                                      ideal_reward=wasserstein_reward, 
                                      ideal_done=done)
                    ...
                    self.constrained_replay_buffer.add(self._last_original_obs, 
                                                   ...
                                                   None,  # nstep suboptimal Q is the accumulative R
                                                   None,  # nstep accumulative R
                                                   wasserstein_reward,  # nstep MC-optimal Q
                                                   None,  # state-action after window_step_th (lower-bound)
                                                   None,  # state-action after window_step_th (lower-bound)
                                                   None,  # check if after window_step_th is terminate state
                                                   None,  # discounted gamma after window_step_th
                                                   )
