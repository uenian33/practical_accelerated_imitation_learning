...
class EmbeddingNetwork(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.embedding = nn.Sequential(nn.Linear(input_size, hidden_size), nn.Tanh(), nn.Linear(
            hidden_size, hidden_size), nn.Tanh(),
            nn.Linear(hidden_size, hidden_size),
            nn.Linear(hidden_size, hidden_size), nn.Tanh(), nn.Linear(hidden_size, input_size))
...
class REDRewarder(object):
    ...
    def train_rewarder(self, imitation_epochs=10, imitation_batch_size=128):
...
class REDDiscriminator(nn.Module):
    ...
    def __init__(self, state_size, action_size, hidden_size, state_only=False):
        ...
        self.gamma = None
        self.predictor = EmbeddingNetwork(state_size * 2 if state_only else state_size + action_size, hidden_size)
        self.target = EmbeddingNetwork(state_size * 2 if state_only else state_size + action_size, hidden_size * 3)
